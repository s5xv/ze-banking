...
export function layout(title, body, { user = null, active = "" } = {}) {
  const inner = user
    ? `<nav>
         ...
         <a href="/app/debits" class="${active === "debits" ? "on" : ""}">Debits</a>
         <a href="/app/borrow" class="${active === "borrow" ? "on" : ""}">Borrow</a>
         <a href="/app/approvals" class="${active === "approvals" ? "on" : ""}">Approvals</a>
